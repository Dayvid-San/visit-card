...
**Content and i18n (PT/EN)**: every visible string lives in `lib/content-registry.ts` (key + `defaultPt`), with English defaults in `lib/content-registry-en.ts`; pages render them through `t(key)` from `useContent()` (`components/content-provider.tsx`), never as JSX literals. `t()` resolves: backend override for the current locale, then the locale's default, then `defaultPt`. The backend answers `en` with `valuePt` for keys that have no `valueEn`, so the provider fetches both locales and drops those, letting the EN default win. A new key needs an entry in both files, or `__tests__/content-registry.test.ts` fails.
